...
const symlink = (server_name) => {
  return new Promise((resolve, reject) => {
    const src = `/etc/nginx/sites-available/${server_name}`;
    const dest = `/etc/nginx/sites-enabled/${server_name}`;
    
    if (!fs.existsSync(src)) {
      return reject(new Error(`Source file ${src} does not exist`));
    }

    exec(`sudo ln -s ${src} ${dest}`, (err, stdout, stderr) => {
      if (err) {
        handleError(err, reject);
        return;
      }
      if (stderr) {
        handleError(stderr, reject);
        return;
      }
